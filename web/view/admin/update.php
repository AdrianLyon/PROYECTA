<?php
    ...
    if ( !empty($_POST)) {
       ...
       /* $fechaFinalContrato = $_POST['fechaFinalContrato'];
        ...
        /*if (empty($fecha)) {
            $fechaError = 'Ingresa tarifa semanal';
            $valid = false;
        }
       ...
       /* if (empty($estatus)) {
        ...
        if ($valid) {
            ...
            $sql = "UPDATE registro  set agencia = ?, nombre = ?, colonia = ?, calle = ?, telefono = ?,  cantidad_prestamos = ?, fecha_pago_programada = ?, fecha_final_contrato = ?, estatus = ? WHERE id = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($agencia, $nombre,$colonia,$calle,$telefono,$cantidadPre,$fecha,$semana4,$estatus,$id));
            Database::disconnect();
            header("Location: index.php");
        ...
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT agencia, nombre, colonia, calle, telefono, cantidad_prestamos, fecha_pago_programada, fecha_final_contrato, estatus FROM registro WHERE id = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($id));
        ...
        $cantidadPre = $data['cantidad_prestamos'];
        $fecha = $data['fecha_pago_programada'];
